  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final intro = details['intro']?.toString() ?? '';
        final branches =
            (details['branches'] as List?)?.cast<Map<String, dynamic>>() ?? [];
        final expectedSalary =
            (details['expectedSalary'] as Map?)?.cast<String, dynamic>() ?? {};
        final types =
            (details['types'] as List?)?.map((e) => e.toString()).toList() ??
            [];
        final futureScope = details['futureScope']?.toString() ?? '';
        final marketRate = details['marketRate']?.toString() ?? '';
        final usefulness = details['usefulness']?.toString() ?? '';
        final roadmap =
            (details['roadmap'] as List?)?.map((e) => e.toString()).toList() ??
            [];
        final imageUrl = data['domainImgUrl']?.toString() ?? '';
